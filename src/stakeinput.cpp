...
bool CFuncStake::InitFromTxIn(const CTxIn& txin)
{
    if (txin.IsZerocoinSpend())
        return error("%s: unable to initialize CFuncStake from zerocoin spend", __func__);
...
bool CFuncStake::SetPrevout(CTransaction txPrev, unsigned int n)
{
    this->txFrom = txPrev;
    this->nPosition = n;
    return true;
...
bool CFuncStake::GetTxFrom(CTransaction& tx) const
{
    if (txFrom.IsNull())
        return false;
    tx = txFrom;
...
bool CFuncStake::GetTxOutFrom(CTxOut& out) const
{
...
CAmount CFuncStake::GetValue() const
{
...
CDataStream CFuncStake::GetUniqueness() const
{
...
bool CFuncStake::ContextCheck(int nHeight, uint32_t nTime)
{
    ...
    if (consensus.NetworkUpgradeActive(nHeight, Consensus::UPGRADE_V3_4) &&
