...
CzFUNCTracker::~CzFUNCTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzFUNCTracker::Init()
{
...
bool CzFUNCTracker::Archive(CMintMeta& meta)
{
...
CMintMeta CzFUNCTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzFUNCTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzFUNCTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CAmount CzFUNCTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzFUNCTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzFUNCTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzFUNCTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzFUNCTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzFUNCTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzFUNCTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzFUNCTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzFUNCTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
void CzFUNCTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzFUNCTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzFUNCTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzFUNCTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
