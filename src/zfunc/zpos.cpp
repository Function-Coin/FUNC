...
bool CLegacyZFuncStake::InitFromTxIn(const CTxIn& txin)
{
    // Construct the stakeinput object
    if (!txin.IsZerocoinSpend())
        return error("%s: unable to initialize CLegacyZFuncStake from non zc-spend", __func__);
    ...
    *this = CLegacyZFuncStake(spend);

    // Find the pindex with the accumulator checksum
    if (!GetIndexFrom())
        return error("%s : Failed to find the block index for zfunc stake origin", __func__);

    // All good
    return true;
}

CLegacyZFuncStake::CLegacyZFuncStake(const libzerocoin::CoinSpend& spend)
{
...
CBlockIndex* CLegacyZFuncStake::GetIndexFrom()
{
...
CAmount CLegacyZFuncStake::GetValue() const
{
    return denom * COIN;
}

CDataStream CLegacyZFuncStake::GetUniqueness() const
{
...
bool CLegacyZFuncStake::ContextCheck(int nHeight, uint32_t nTime)
{
    const Consensus::Params& consensus = Params().GetConsensus();
    if (!consensus.NetworkUpgradeActive(nHeight, Consensus::UPGRADE_ZC_V2) || nHeight >= consensus.height_last_ZC_AccumCheckpoint)
        return error("%s : zFUNC stake block: height %d outside range", __func__, nHeight);
