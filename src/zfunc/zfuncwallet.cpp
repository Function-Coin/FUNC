...
CzFUNCWallet::CzFUNCWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zfunc seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZFUNCSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZFUNCSeed_deprecated()) {
                    LogPrintf("%s: Updated zFUNC seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zfunc seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zfunc wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzFUNCWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZFUNCCount(nCountLastUsed);
    else if (!walletdb.ReadZFUNCCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzFUNCWallet::Lock()
{
    seedMaster = 0;
}

void CzFUNCWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzFUNCWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZFUNC(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzFUNCWallet::LoadMintPoolFromDB()
{
...
void CzFUNCWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzFUNCWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzFUNCWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (std::pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zfuncTracker->HasPubcoinHash(pMint.first)) {
...
bool CzFUNCWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZFUNC(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zfuncTracker which also adds to database
    pwalletMain->zfuncTracker->Add(dMint, true);
...
void CzFUNCWallet::SeedToZFUNC(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzFUNCWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzFUNCWallet::GenerateDeterministicZFUNC(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzFUNCWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzFUNCWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzFUNCWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
