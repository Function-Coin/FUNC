...
void FillBlockPayee(CMutableTransaction& txNew, CAmount nFees, bool fProofOfStake, bool fZFUNCStake)
{
    CBlockIndex* pindexPrev = chainActive.Tip();
    ...
    if (sporkManager.IsSporkActive(SPORK_13_ENABLE_SUPERBLOCKS) && budget.IsBudgetPaymentBlock(pindexPrev->nHeight + 1)) {
        ...
        masternodePayments.FillBlockPayee(txNew, nFees, fProofOfStake, fZFUNCStake);
    }
}
...
void CMasternodePayments::FillBlockPayee(CMutableTransaction& txNew, int64_t nFees, bool fProofOfStake, bool fZFUNCStake)
{
    CBlockIndex* pindexPrev = chainActive.Tip();
    if (!pindexPrev) return;
    ...
    CAmount operationsPayment = GetOperationsPayment(pindexPrev->nHeight+1, blockValue);
    CAmount nonPoSPayment = masternodePayment + operationsPayment;
    ...
    if (hasPayment) {
        if (fProofOfStake) {
            ...
            if (operationsPayment <= 0) {
                txNew.vout.resize(i + 1);
                txNew.vout[i].scriptPubKey = payee;
                txNew.vout[i].nValue = masternodePayment;
            } else {
                txNew.vout.resize(i + 2);
                txNew.vout[i].scriptPubKey = OpFundScriptPubKey();
                txNew.vout[i].nValue = operationsPayment;
                txNew.vout[i+1].scriptPubKey = payee;
            ...
            if (!txNew.vout[1].IsZerocoinMint()) {
                if (i == 2) {
                    ...
                    CAmount mnPaymentSplit = nonPoSPayment / outputs;
                    CAmount mnPaymentRemainder = nonPoSPayment - (mnPaymentSplit * outputs);
...
bool CMasternodeBlockPayees::IsTransactionValid(const CTransaction& txNew)
{
    ...
    CAmount requiredMasternodePayment2 = GetMasternodePayment(nBlockHeight-2, nReward2);
