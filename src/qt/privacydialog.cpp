                                                          ...
                                                          fDenomsMinimized(true)
{
    ...
    // "Spending 999999 zFUNC ought to be enough for anybody." - Bill Gates, 2017
    ui->zFUNCpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzFUNCSyncStatus->setText("(" + tr("out of sync") + ")");
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zFUNCpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzFUNC_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zFUNC is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Mint_zFUNC, true));
        if (!ctx.isValid()) {
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zFUNC...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zFUNC in ") +
...
void PrivacyDialog::on_pushButtonSpendzFUNC_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zFUNC is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Send_zFUNC, true));
        if (!ctx.isValid()) {
            // Unlock wallet was cancelled
            return;
        }
        // Wallet is unlocked now, sedn zFUNC
        sendzFUNC();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zFUNC
    sendzFUNC();
}

void PrivacyDialog::on_pushButtonZFuncControl_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;

    ZFuncControlDialog* zFuncControl = new ZFuncControlDialog(this);
    zFuncControl->setModel(walletModel);
    zFuncControl->exec();
}

void PrivacyDialog::setZFuncControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzFuncSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzFUNC()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Func Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zFUNCpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zFUNC is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " FUNC </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zFUNC</b>";
    ...
    // use mints from zFUNC selector if applicable
    vector<CMintMeta> vMintsToFetch;
    vector<CZerocoinMint> vMintsSelected;
    if (!ZFuncControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZFuncControlDialog::GetSelectedMints();
    ...
    if (walletModel && walletModel->getAddressTableModel()) {
        // If zFUNC was spent successfully update the addressbook with the label
    ...
    // Clear zfunc selector in case it was used
    ZFuncControlDialog::setSelectedMints.clear();
    ui->labelzFuncSelected_int->setText(QString("0"));
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zFUNC Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Func, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zFUNC Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    std::vector<CMintMeta> vMints = pwalletMain->zfuncTracker->GetMints(false);
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zFUNC </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zFUNC "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zFUNC "));
    ui->labelzAvailableAmount_4->setText(QString::number(zerocoinBalance/COIN) + QString(" zFUNC "));
    ui->labelzFUNCAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
    ...
    ui->labelZsupplyAmount->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zFUNC </b> "));
    ui->labelZsupplyAmount_2->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zFUNC </b> "));
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        ...
        QString strSupply = QString::number(nSupply) + " x " + QString::number(denom) + " = <b>" +
                            QString::number(nSupply*denom) + " zFUNC </b> ";
        switch (denom) {
            case libzerocoin::CoinDenomination::ZQ_ONE:
...
{
    ui->labelzFUNCSyncStatus->setVisible(fShow);
}
...
void PrivacyDialog::updateSPORK16Status()
{
    ...
    bool fButtonsEnabled =  ui->pushButtonMintzFUNC->isEnabled();
    ...
    if (fMaintenanceMode && fButtonsEnabled) {
        // Mint zFUNC
        ui->pushButtonMintzFUNC->setEnabled(false);
        ui->pushButtonMintzFUNC->setToolTip(tr("zFUNC is currently disabled due to maintenance."));

        // Spend zFUNC
        ui->pushButtonSpendzFUNC->setEnabled(false);
        ui->pushButtonSpendzFUNC->setToolTip(tr("zFUNC is currently disabled due to maintenance."));
    } else if (!fMaintenanceMode && !fButtonsEnabled) {
        // Mint zFUNC
        ui->pushButtonMintzFUNC->setEnabled(true);
        ui->pushButtonMintzFUNC->setToolTip(tr("PrivacyDialog", "Enter an amount of FUNC to convert to zFUNC", 0));

        // Spend zFUNC
        ui->pushButtonSpendzFUNC->setEnabled(true);
        ui->pushButtonSpendzFUNC->setToolTip(tr("Spend Zerocoin. Without 'Pay To:' address creates payments to yourself."));
    }
}
