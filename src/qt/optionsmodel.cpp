...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::FUNC);
    ...
    if (settings.contains("nAnonymizeFuncAmount") || reset)
        SoftSetArg("-anonymizefuncamount", settings.value("nAnonymizeFuncAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in func.cpp during shotdown to also remove the window positions
