...
#include "qt/func/forms/ui_snackbar.h"
#include "qt/func/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(FUNCGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &FUNCGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
