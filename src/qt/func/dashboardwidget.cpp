...
#include "qt/func/dashboardwidget.h"
#include "qt/func/forms/ui_dashboardwidget.h"
#include "qt/func/sendconfirmdialog.h"
#include "qt/func/txrow.h"
...
DashboardWidget::DashboardWidget(FUNCGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    setCssProperty(ui->labelSquareFunc, "square-chart-func");
    setCssProperty(ui->labelSquarezFunc, "square-chart-zfunc");
    setCssProperty(ui->labelFunc, "text-chart-func");
    setCssProperty(ui->labelZfunc, "text-chart-zfunc");
    ...
    setCssProperty(ui->labelAmountFunc, "text-stake-func-disable");
    setCssProperty(ui->labelAmountZfunc, "text-stake-zfunc-disable");
...
bool hasCharts = false;
    ...
    if (window)
        connect(window, &FUNCGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
    if (walletModel && walletModel->getOptionsModel()) {
        ...
        connect(ui->pushImgEmpty, &QPushButton::clicked, window, &FUNCGUI::openFAQ);
        connect(ui->btnHowTo, &QPushButton::clicked, window, &FUNCGUI::openFAQ);
...
const QMap<int, std::pair<qint64, qint64>> DashboardWidget::getAmountBy()
{
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool isFunc = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::StakeZFUNC;
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair FUNC, zFUNC
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            func = (pair.first != 0) ? pair.first / 100000000 : 0;
            zfunc = (pair.second != 0) ? pair.second / 100000000 : 0;
            chartData->totalFunc += pair.first;
            chartData->totalZfunc += pair.second;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesFunc.append(func);
        chartData->valueszFunc.append(zfunc);

        int max = std::max(func, zfunc);
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->setColor(QColor(0,179,62));
    set1->setColor(QColor(128,255,172));
    ...
    set0->append(chartData->valuesFunc);
    set1->append(chartData->valueszFunc);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalFunc > 0 || chartData->totalZfunc > 0) {
        setCssProperty(ui->labelAmountFunc, "text-stake-func");
        setCssProperty(ui->labelAmountZfunc, "text-stake-zfunc");
    } else {
        setCssProperty(ui->labelAmountFunc, "text-stake-func-disable");
        setCssProperty(ui->labelAmountZfunc, "text-stake-zfunc-disable");
    }
    forceUpdateStyle({ui->labelAmountFunc, ui->labelAmountZfunc});
    ui->labelAmountFunc->setText(GUIUtil::formatBalance(chartData->totalFunc, nDisplayUnit));
    ui->labelAmountZfunc->setText(GUIUtil::formatBalance(chartData->totalZfunc, nDisplayUnit, true));
