...
#include "ui_zfunccontroldialog.h"
...
std::set<std::string> ZFuncControlDialog::setSelectedMints;
std::set<CMintMeta> ZFuncControlDialog::setMints;

bool CZFuncControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZFuncControlDialog::COLUMN_DENOMINATION || column == ZFuncControlDialog::COLUMN_VERSION || column == ZFuncControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZFuncControlDialog::ZFuncControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZFuncControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZFuncControlDialog::~ZFuncControlDialog()
{
    delete ui;
}

void ZFuncControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZFuncControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZFuncControlWidgetItem* itemDenom(new CZFuncControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZFuncControlWidgetItem *itemMint = new CZFuncControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        {
            LOCK(pwalletMain->zfuncTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zfuncTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zFUNC.";
            else if (!mint.isSeedCorrect)
                strReason = "The zFUNC seed used to mint this zFUNC is not the same as currently hold in the wallet";
            else
...
void ZFuncControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZFuncControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZFunc_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZFuncControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZFuncControlDialog::GetSelectedMints()
{
...
void ZFuncControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
