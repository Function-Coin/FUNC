...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake, int nPreviousBlockHeight)
{
    ...
    if (txin.IsZerocoinSpend()) {
        ...
        stake = std::unique_ptr<CStakeInput>(new CZFuncStake(spend));

        if (!ContextualCheckZerocoinStake(nPreviousBlockHeight, stake.get()))
            return error("%s : staked zFUNC fails context checks", __func__);
    } else {
        ...
        CFuncStake* funcInput = new CFuncStake();
        funcInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(funcInput);
    }
    ...
    if (!txin.IsZerocoinSpend() && nPreviousBlockHeight >= Params().Zerocoin_Block_Public_Spend_Enabled() - 1
        ...
        //Equivalent for zFUNC is checked above in ContextualCheckZerocoinStake()
