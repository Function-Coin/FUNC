...
bool ContextualCheckZerocoinSpendNoSerialCheck(const CTransaction& tx, const libzerocoin::CoinSpend* spend, CBlockIndex* pindex, const uint256& hashBlock)
{
    ...
    if (pindex->nHeight >= Params().Zerocoin_Block_Public_Spend_Enabled()) {
        ...
        if (v1Serial) {
            return error("%s : zFUNC v1 serial spend not spendable, serial %s, tx %s\n", __func__,
...
bool CheckZerocoinSpend(const CTransaction& tx, bool fVerifySignature, CValidationState& state, bool fFakeSerialAttack)
{
    ...
    for (const CTxIn& txin : tx.vin) {
        ...
        if (isPublicSpend) {
            ...
            if (!ZFUNCModule::parseCoinSpend(txin, tx, prevOut, publicSpend)){
        ...
        if (isPublicSpend) {
            ...
            if (!ZFUNCModule::validateInput(txin, prevOut, tx, ret)){
        ...
        if (tx.HasZerocoinSpendInputs()) {
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (isPublicSpend) {
                    libzerocoin::ZerocoinParams* params = Params().Zerocoin_Params(false);
                    PublicCoinSpend publicSpend(params);
                    if (!ZFUNCModule::ParseZerocoinPublicSpend(txIn, tx, state, publicSpend)){
                        return false;
                    }
            ...
            // Check that zFUNC mints (if included) are not already known
        ...
        for (const CTxIn& in : tx.vin) {
            ...
            if (in.IsZerocoinSpend() || isPublicSpend) {
                ...
                if (isPublicSpend) {
                    ...
                    if (!ZFUNCModule::ParseZerocoinPublicSpend(in, tx, state, publicSpend)){
...
bool CheckWork(const CBlock block, CBlockIndex* const pindexPrev)
{
    ...
    if (block.nBits != nBitsRequired) {
        // Func Specific reference to the block with the wrong threshold was used.
        if ((block.nTime == Params().FuncBadBlockTime()) && (block.nBits == Params().FuncBadBlocknBits())) {
            // accept FUNC block minted with incorrect proof of work threshold
            return true;
        }
        ...
        if (isBlockFromFork) {
            ...
            if(hasZFUNCInputs){
                for (const CTxIn& zFuncInput : zFUNCInputs) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(zFuncInput);
