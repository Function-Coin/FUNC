...
/** Fees smaller than this (in ufunc) are considered zero fee (for relaying, mining and transaction creation)
...

    {
        ...
        if (hasZcSpendInputs) {
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("%s : zFUNC spend tx %s already in block %d", __func__, tx.GetHash().GetHex(), nHeightTx),
            ...
            // Reject legacy zFUNC mints
            if (!Params().IsRegTestNet() && tx.HasZerocoinMintOutputs())
                return state.Invalid(error("%s : tried to include zFUNC mint output in tx %s",
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight == 0) {
         ...
         nSubsidy = 18.5 * COIN;
    } else if (nHeight <= 310000 && nHeight > 242640) {
         nSubsidy = 19 * COIN;
    } else if (nHeight <= 310009 && nHeight > 310000) {
         nSubsidy = 12 * COIN;
    } else if (nHeight <= 310010 && nHeight > 310009) {
         nSubsidy = 200010 * COIN;
    } else {
         nSubsidy = 12 * COIN;
    }
...
int64_t GetOperationsPayment(int nHeight, int64_t blockValue)
{
    int64_t ret = 0;
    ...
    if (nHeight <= 310000) {
         return 0;
    } else if (blockValue > 200000 * COIN) {
         ret = 200000 * COIN;
    } else {
         ret = 2 * COIN;
    } 
...
{
    // Set the Dev reward address
    std::string devPayoutAddr  = Params().GetConsensus().strDevPayoutAddr;

    return GetScriptForDestination(DecodeDestination(devPayoutAddr));
}
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    //Track zFUNC money supply
    if (!UpdateZFUNCSupplyDisconnect(block, pindex))
        return error("%s: Failed to calculate new zFUNC supply", __func__);
...
{
    util::ThreadRename("func-scriptch");
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    if (!fAlreadyChecked && !CheckBlock(block, state, !fJustCheck, !fJustCheck)) {
        if (pindex->pprev != NULL && state.CorruptionPossible()) {
    ...
    // Update zFUNC money supply map
    if (!UpdateZFUNCSupplyConnect(block, pindex, fJustCheck)) {
        return state.DoS(100, error("%s: Failed to calculate new zFUNC supply for block=%s height=%d", __func__,
                                    block.GetHash().GetHex(), pindex->nHeight), REJECT_INVALID);
    }

    // A one-time event where the zFUNC supply was off (due to serial duplication off-chain on main net)
    if (Params().NetworkID() == CBaseChainParams::MAIN && pindex->nHeight == consensus.height_last_ZC_WrappedSerials + 1
        ...
        RecalculateFUNCSupply(consensus.vUpgrades[Consensus::UPGRADE_ZC].nActivationHeight, false);
    }
...
bool CheckColdStakeFreeOutput(const CTransaction& tx, const int nHeight)
{
    ...
    if (outs >=4 && lastOut.scriptPubKey != tx.vout[outs-3].scriptPubKey) {
...
bool CheckWork(const CBlock block, CBlockIndex* const pindexPrev)
{
    ...
    if (block.nBits != nBitsRequired) {
        ...
        const Consensus::Params& consensus = Params().GetConsensus();
        if ((block.nTime == (uint32_t) consensus.nFuncBadBlockTime) &&
                (block.nBits == (uint32_t) consensus.nFuncBadBlockBits)) {
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, CBlockIndex* const pindexPrev)
{
    ...
    if (nHeight > 10000) { // pindexPrev is only null on the first block which is a version 1 block.
...
bool AcceptBlock(const CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (isPoS) {
        ...
        if (operationsPayment > 0 && sporkManager.IsSporkActive(SPORK_8_MASTERNODE_PAYMENT_ENFORCEMENT)) {
          ...
          if (block.vtx[1].vout[s - 2].scriptPubKey != OpFundScriptPubKey() || block.vtx[1].vout[s - 2].nValue != operationsPayment)
            ...
            while (!chainActive.Contains(prev)) {
                ...
                for (const CTransaction& t : bl.vtx) {
                    ...
                    for (const CTxIn& in: t.vin) {
                        ...
                        if(hasZFUNCInputs && in.IsZerocoinSpend())
...
bool static ProcessMessage(CNode* pfrom, std::string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == NetMsgType::VERSION) {
        ...
        if (abs64(nTimeOffset) < nTimeSlotLength) {
