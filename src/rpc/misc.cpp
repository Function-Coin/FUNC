...
UniValue getinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"balance\": xxxxxxx,           (numeric) the total func balance of the wallet (excluding zerocoins)\n"
            ...
            "  \"zFUNCsupply\" :\n"
            "  {\n"
            "     \"1\" : n,            (numeric) supply of 1 zFUNC denomination\n"
            "     \"5\" : n,            (numeric) supply of 5 zFUNC denomination\n"
            "     \"10\" : n,           (numeric) supply of 10 zFUNC denomination\n"
            "     \"50\" : n,           (numeric) supply of 50 zFUNC denomination\n"
            "     \"100\" : n,          (numeric) supply of 100 zFUNC denomination\n"
            "     \"500\" : n,          (numeric) supply of 500 zFUNC denomination\n"
            "     \"1000\" : n,         (numeric) supply of 1000 zFUNC denomination\n"
            "     \"5000\" : n,         (numeric) supply of 5000 zFUNC denomination\n"
            "     \"total\" : n,        (numeric) The total supply of all zFUNC denominations\n"
            "  }\n"
            "  \"keypoololdest\": xxxxxx,      (numeric) the timestamp (seconds since GMT epoch) of the oldest pre-generated key in the key pool\n"
            "  \"keypoolsize\": xxxx,          (numeric) how many new keys are pre-generated\n"
            "  \"unlocked_until\": ttt,        (numeric) the timestamp in seconds since epoch (midnight Jan 1 1970 GMT) that the wallet is unlocked for transfers, or 0 if the wallet is locked\n"
            "  \"paytxfee\": x.xxxx,           (numeric) the transaction fee set in func/kb\n"
            "  \"relayfee\": x.xxxx,           (numeric) minimum relay fee for non-free transactions in func/kb\n"
    ...
    UniValue zfuncObj(UniValue::VOBJ);
    for (auto denom : libzerocoin::zerocoinDenomList) {
        if (mapZerocoinSupply.empty())
            zfuncObj.push_back(Pair(std::to_string(denom), ValueFromAmount(0)));
        else
            zfuncObj.push_back(Pair(std::to_string(denom), ValueFromAmount(mapZerocoinSupply.at(denom) * (denom*COIN))));
    }
    zfuncObj.push_back(Pair("total", ValueFromAmount(GetZerocoinSupply())));
    obj.push_back(Pair("zFUNCsupply", zfuncObj));
...
UniValue validateaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw std::runtime_error(
            "validateaddress \"funcaddress\"\n"
            "\nReturn information about the given func address.\n"

            "\nArguments:\n"
            "1. \"funcaddress\"     (string, required) The func address to validate\n"

            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"funcaddress\",    (string) The func address validated\n"
            "  \"scriptPubKey\" : \"hex\",       (string) The hex encoded scriptPubKey generated by the address\n"
            "  \"ismine\" : true|false,          (boolean) If the address is yours or not\n"
            "  \"isstaking\" : true|false,       (boolean) If the address is a staking address for FUNC cold staking\n"
...
#ifdef ENABLE_WALLET
        // Case 1: FUNC address and we have full public key:
...
UniValue createmultisig(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2)
        throw std::runtime_error(
            ...
            "2. \"keys\"       (string, required) A json array of keys which are func addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) func address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw std::runtime_error(
            "verifymessage \"funcaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"

            "\nArguments:\n"
            "1. \"funcaddress\"  (string, required) The func address to use for the signature.\n"
...
UniValue getstakingstatus(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"staking_enabled\": true|false,     (boolean) whether staking is enabled/disabled in func.conf\n"
            "  \"coldstaking_enabled\": true|false, (boolean) whether cold-staking is enabled/disabled in func.conf\n"
            "  \"haveconnections\": true|false,     (boolean) whether network connections are present\n"
            "  \"mnsync\": true|false,              (boolean) whether the required masternode/spork data is synced\n"
            "  \"walletunlocked\": true|false,      (boolean) whether the wallet is unlocked\n"
            "  \"stakeablecoins\": n                (numeric) number of stakeable UTXOs\n"
            "  \"stakingbalance\": d                (numeric) FUNC value of the stakeable coins (minus reserve balance, if any)\n"
