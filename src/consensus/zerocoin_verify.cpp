...
bool RecalculateFUNCSupply(int nHeightStart, bool fSkipZfunc)
{
    ...
    while (true) {
        ...
        // Rewrite zfunc supply too
        if (!fSkipZfunc && consensus.NetworkUpgradeActive(pindex->nHeight, Consensus::UPGRADE_ZC)) {
            UpdateZFUNCSupplyConnect(block, pindex, true);
        }
