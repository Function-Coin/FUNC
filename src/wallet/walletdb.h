...
public:
    ...
    bool WriteZFUNCSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadZFUNCSeed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadZFUNCSeed_deprecated(uint256& seed);
    bool EraseZFUNCSeed();
    bool EraseZFUNCSeed_deprecated();

    bool WriteZFUNCCount(const uint32_t& nCount);
    bool ReadZFUNCCount(uint32_t& nCount);
