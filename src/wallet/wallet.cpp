        ...
        if (zwalletMain) {
            ...
            if (CWalletDB(strWalletFile).ReadCurrentSeedHash(hashSeed)) {
                ...
                if (!GetDeterministicSeed(hashSeed, nSeed)) {
                    return error("Failed to read zFUNC seed from DB. Wallet is probably corrupt.");
                }
...
int CWallet::ScanForWalletTransactions(CBlockIndex* pindexStart, bool fUpdate, bool fromStartup)
{
    ...
    CBlockIndex* pindex = pindexStart;
    {
        ...
        while (pindex) {
            ...
            // Will try to rescan it if zFUNC upgrade is active.
            doZFuncRescan(pindex, block, setAddedToWallet, consensus, fCheckZFUNC);
                             ...
                             ) const
{
...

    {
        ...
        for (std::map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it) {
            ...
            const Consensus::Params& consensus = Params().GetConsensus();
            if (nCoinType == STAKEABLE_COINS && nDepth <= Params().GetConsensus().nStakeMinDepth && consensus.NetworkUpgradeActive(chainActive.Height(), Consensus::UPGRADE_V3_4)) continue;
            if (nCoinType == STAKEABLE_COINS && nDepth <= Params().GetConsensus().nCoinbaseMaturity && !consensus.NetworkUpgradeActive(chainActive.Height(), Consensus::UPGRADE_V3_4)) continue;
...
bool CWallet::GetBudgetSystemCollateralTX(CWalletTx& tx, uint256 hash, bool useIX)
{
    ...
    vecSend.push_back(CRecipient{scriptChange, BUDGET_FEE_TX_OLD, false}); // Old 50 FUNC collateral
...
bool CWallet::GetBudgetFinalizationCollateralTX(CWalletTx& tx, uint256 hash, bool useIX)
{
    ...
    std::string strFail = "";
    std::vector<CRecipient> vecSend;
    vecSend.push_back(CRecipient{scriptChange, BUDGET_FEE_TX, false}); // New 5 FUNC collateral
