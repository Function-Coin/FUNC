    ...
    if(IsCrypted()) {
        if (!IsLocked()) { //if we have password
            ...
            if (EncryptSecret(vMasterKey, kmSeed, hashSeed, vchSeedSecret)) {
                ...
                if (db.WriteZFUNCSeed(hashSeed, vchSeedSecret)) {
                    return true;
                }
            }
            strErr = "encrypt seed";
        ...
        if (db.WriteZFUNCSeed(hashSeed, ToByteVector(seed))) {
            return true;
        }
        strErr = "save zfuncseed to wallet";
    }
    //the use case for this is no password set seed, mint dzFUNC,
...
bool CWallet::GetDeterministicSeed(const uint256& hashSeed, uint256& seedOut)
{
    ...
    if (IsCrypted()) {
        if(!IsLocked()) { //if we have password
            ...
            //read encrypted seed
            if (db.ReadZFUNCSeed(hashSeed, vchCryptedSeed)) {
        ...
        // wallet not crypted
        if (db.ReadZFUNCSeed(hashSeed, vchSeed)) {
...
void CWallet::doZFuncRescan(const CBlockIndex* pindex, const CBlock& block,
        std::set<uint256>& setAddedToWallet, const Consensus::Params& consensus, bool fCheckZFUNC)
{
    //If this is a zapwallettx, need to read zfunc
    if (fCheckZFUNC && consensus.NetworkUpgradeActive(pindex->nHeight, Consensus::UPGRADE_ZC)) {
                                    ...
                                    CZerocoinSpendReceipt& receipt, libzerocoin::SpendType spendType, CBlockIndex* pindexCheckpoint)
{
    ...
    for (auto &it : mapMintsSelected) {
        ...
        CTxIn in;
        if(!ZFUNCModule::createInput(in, mint, hashTxOut, spendVersion)) {
        ...
        CTxDestination * changeAddress)
{
    ...
    const int nMaxSpends = Params().GetConsensus().ZC_MaxPublicSpendsPerTx; // Maximum possible spends for one zFUNC public spend transaction
...
void CWallet::setZWallet(CzFUNCWallet* zwallet)
{
    zwalletMain = zwallet;
    zfuncTracker = std::unique_ptr<CzFUNCTracker>(new CzFUNCTracker(this));
}

CzFUNCWallet* CWallet::getZWallet()
{
    return zwalletMain;
}
