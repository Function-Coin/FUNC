...
extern bool fGlobalUnlockSpendCache; // Bool used for letting the precomputing thread know that zfuncspends need to use the cs_spendcache
...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 FUNC at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zFUNC send
enum ZerocoinSpendStatus {
    ZFUNC_SPEND_OKAY = 0,                            // No error
    ZFUNC_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZFUNC_WALLET_LOCKED = 2,                         // Wallet was locked
    ZFUNC_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZFUNC_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZFUNC_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZFUNC_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZFUNC_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZFUNC_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZFUNC_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZFUNC_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZFUNC_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZFUNC_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZFUNC_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZFUNC_SPENT_USED_ZFUNC = 14,                      // Coin has already been spend
    ZFUNC_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZFUNC_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void ZFuncBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZFUNCOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    std::string GetUniqueWalletBackupName(bool fzfuncAuto) const;
    ...
    */
    ...
    bool fBackupMints;
    std::unique_ptr<CzFUNCTracker> zfuncTracker;
    ...
    void setZWallet(CzFUNCWallet* zwallet)
    {
        zwalletMain = zwallet;
        zfuncTracker = std::unique_ptr<CzFUNCTracker>(new CzFUNCTracker(strWalletFile));
    }

    CzFUNCWallet* getZWallet() { return zwalletMain; }
    ...
    void setZFuncAutoBackups(bool fEnabled)
    {
    ...
    /** zFUNC reset */
    boost::signals2::signal<void()> NotifyzFUNCReset;
