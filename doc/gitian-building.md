...
*Setup instructions for a gitian build of FUNC Core using a VM or physical system.*
...
to the FUNC GitHub release page.
...
- [Building FUNC Core](#building-func-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/func-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/func-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/func-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/func-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/func-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/func-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/CryptoDev-Project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@func.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/func-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/func-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/func-osx-*-build.assert
```
