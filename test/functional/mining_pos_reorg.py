...
class ReorgStakeTest(FuncTestFramework):
    ...
    def check_money_supply(self, expected_func, expected_zfunc):
        g_info = [self.nodes[i].getinfo() for i in range(self.num_nodes)]
        # verify that nodes have the expected FUNC and zFUNC supply
        for node in g_info:
            assert_equal(node['moneysupply'], DecimalAmt(expected_func))
            for denom in node['zFUNCsupply']:
                assert_equal(node['zFUNCsupply'][denom], DecimalAmt(expected_zfunc[denom]))
    ...
    def run_test(self):
        ...
        # Check FUNC and zFUNC supply at the beginning
        # ------------------------------------------
        # zFUNC supply: 2 coins for each denomination
        expected_zfunc_supply = {
            "1": 2,
            "5": 10,
            "10": 20,
            "50": 100,
        ...
        # FUNC supply: block rewards minus burned fees for minting
        expected_money_supply = 250.0 * 330 - 16 * 0.01
        self.check_money_supply(expected_money_supply, expected_zfunc_supply)
        ...
        # Verify that FUNC and zFUNC supplies were properly updated after the spends and reorgs
        self.log.info("Check FUNC and zFUNC supply...")
        ...
        expected_zfunc_supply[str(spent_coin_0)] -= spent_coin_0
        expected_zfunc_supply[str(spent_coin_1)] -= spent_coin_1
        expected_zfunc_supply["total"] -= (spent_coin_0 + spent_coin_1)
        self.check_money_supply(expected_money_supply, expected_zfunc_supply)
