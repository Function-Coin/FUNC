...
class ZerocoinSpendTest(FuncTestFramework):
    ...
    def run_test(self):
        ...
        def check_balances(denom, zfunc_bal, func_bal):
            zfunc_bal -= denom
            assert_equal(self.nodes[2].getzerocoinbalance()['Total'], zfunc_bal)
            func_bal += denom
            wi = self.nodes[2].getwalletinfo()
            assert_equal(wi['balance'] + wi['immature_balance'], func_bal)
            return zfunc_bal, func_bal
        ...
        zfunc_balance = self.nodes[2].getzerocoinbalance()['Total']
        assert_equal(balance, DecimalAmt(13833.92))
        assert_equal(zfunc_balance, 6666)
        ...
        zfunc_balance, balance = check_balances(denom_2, zfunc_balance, balance)
        ...
        block_time = stake_4_blocks(block_time)
        self.check_tx_in_chain(0, txid)
        zfunc_balance, balance = check_balances(denom_3, zfunc_balance, balance)
        ...
        _, _ = check_balances(denom_1, zfunc_balance, balance)
        self.log.info("--> VALID PUBLIC COIN SPEND (v3) PASSED")
